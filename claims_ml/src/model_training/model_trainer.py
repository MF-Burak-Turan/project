...
class ModelTrainer(BaseEstimator):
    ...
    def _validate_model(self) -> None:
        ...
        if not hasattr(self.model, 'fit'):
            raise ValueError(
                ...
                "Ensure your model followst he scikit-learn API."
            )
    ...
    def predict(self, X:Union[np.ndarray, pd.DataFrame]) -> np.ndarray:
        ...
        if X.shape[1] != self.n_feature_in_:
            raise ValueError(
                f"X has {X.shape[1]} feature, but ModelTrainer is expecting"
                f"{self.n_feature_in_} features as seen in fit."
            )
        try:
